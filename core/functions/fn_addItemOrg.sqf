...
for "_x" from 1 to _amount do {
	...
	if (_position isEqualTo "none") then {
		...
		if (!_success && {
			_type in ["Map", "GPS", "Compass", "Watch", "NVGoggles"]
		}) then {
		...
		if (!_success && {
			_type in ["AccessoryMuzzle", "AccessoryPointer", "AccessorySights", "AccessoryBipod"]
		}) then {
