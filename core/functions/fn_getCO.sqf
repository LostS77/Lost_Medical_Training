...
private _manualOverride = switch (_side) do {
	case blufor: {
		GVAR(CoC_ManualOverride_Blufor)
	};
	case opfor: {
		GVAR(CoC_ManualOverride_Opfor)
	};
	case independent: {
		GVAR(CoC_ManualOverride_Indfor)
	};
	case civilian: {
		GVAR(CoC_ManualOverride_Civfor)
	};
};
...
if (_filteredManualOverride isEqualTo []) then {
	// default behaviour
...
if (_oldCO isNotEqualTo _co) then {
	// if the CO has changed
