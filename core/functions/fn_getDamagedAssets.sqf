	...
	if ((GETVAR(_x, AssetTeam, sideEmpty)) isEqualTo _team) then {
		...
		if (alive _x) then {
			if (!canMove _x && {
				...
				_x getHitPointDamage "hitEngine" >= 0.5
			}) then {
				private _searchIndex = _disabledAssets findIf {
					(_x select 0) isEqualTo _assetName
				};
			...
			private _searchIndex = _destroyedAssets findIf {
				(_x select 0) isEqualTo _assetName
			};
			if (_searchIndex isEqualTo -1) then {
