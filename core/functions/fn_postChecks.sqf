...
[{
	...
	private _moduleMenus = {
		(_x select 0) in _modulesWithMenus
	} count GVAR(Modules);
	LOG_1("_moduleMenus: %1", _moduleMenus);
	[{
		_this isEqualTo 0 || ((((allDiarySubjects player) select {
			_x select 0 isEqualTo QGVAR(Menu)
		}) select 0 select 3) isEqualTo _this)
	}, {
