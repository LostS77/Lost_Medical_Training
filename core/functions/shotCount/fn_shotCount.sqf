...
switch (_side) do {
	case west: {
		private _findIndex = GVAR(west_ExpendedAmmo) findIf {
			_x isEqualTo _magazine
		};
	...
	case east: {
		private _findIndex = GVAR(east_ExpendedAmmo) findIf {
			_x isEqualTo _magazine
		};
		if (_findIndex isEqualTo -1) then {
	...
	case independent: {
		private _findIndex = GVAR(ind_ExpendedAmmo) findIf {
			_x isEqualTo _magazine
		};
		if (_findIndex isEqualTo -1) then {
	...
	case civilian: {
		private _findIndex = GVAR(civ_ExpendedAmmo) findIf {
			_x isEqualTo _magazine
		};
		if (_findIndex isEqualTo -1) then {
