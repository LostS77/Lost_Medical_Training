...
if (west in GVAR(TeamSides) && {
	GETMVAR(EndScreenDisplay_West, true)
}) then {
	_textArray pushBack [_textBLUFOR, GVAR(west_ExpendedAmmo)];
};
if (east in GVAR(TeamSides) && {
	GETMVAR(EndScreenDisplay_East, true)
}) then {
	_textArray pushBack [_textOPFOR, GVAR(east_ExpendedAmmo)];
};
if (independent in GVAR(TeamSides) && {
	GETMVAR(EndScreenDisplay_Ind, true)
}) then {
	_textArray pushBack [_textINDFOR, GVAR(ind_ExpendedAmmo)];
};
if (civilian in GVAR(TeamSides) && {
	GETMVAR(EndScreenDisplay_Civ, true)
}) then {
