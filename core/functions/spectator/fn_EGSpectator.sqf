...
switch _mode do {
	...
	case "onLoad": {
		...
		uiNamespace setVariable [VAR_TICK, addMissionEventHandler ["Draw3D",
			...
			uiNamespace setVariable [VAR_KEY_DOWN, _display displayaddeventhandler ["KeyDown", {
				["KeyDown", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_KEY_UP, _display displayaddeventhandler ["KeyUp", {
				["KeyUp", _this] call DISPLAY;
			}]];

			// Mouse input handling
			uiNamespace setVariable [VAR_MOUSE_BUTTON_DOWN, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MOUSEHANDLER) ctrlAddEventHandler ["MouseButtonDown", {
				["MouseButtonDown", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_MOUSE_BUTTON_UP, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MOUSEHANDLER) ctrlAddEventHandler ["MouseButtonUp", {
				["MouseButtonUp", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_MOUSE_DBL_BUTTON_DOWN, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MOUSEHANDLER) ctrlAddEventHandler ["MouseButtonDblClick", {
				["MouseButtonDblClick", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_MOUSE_Z_CHANGED, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MOUSEHANDLER) ctrlAddEventHandler ["MouseZChanged", {
				["MouseZChanged", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_MOUSE_MOVING, _display displayAddEventHandler ["MouseMoving", {
				["MouseMoving", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_IS_HOLDING_RIGHT_M_BUTTON, false];

			// Add event for children being destroyed
			uiNamespace setVariable [VAR_CHILD_DESTROYED, _display displayaddeventhandler ["ChildDestroyed", {
				["ChildDestroyed", _this] call DISPLAY;
			}]];

			// The tree list event handlers
			uiNamespace setVariable [VAR_TREE_MOUSE_ENTER, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_LIST) ctrlAddEventHandler ["MouseEnter", {
				["ShowList", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_TREE_MOUSE_EXIT, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_LIST) ctrlAddEventHandler ["MouseExit", {
				["HideList", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_TREE_PLAYER_CLICKED, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_LIST) ctrlAddEventHandler ["TreeSelChanged", {
				["TreeClicked", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_TREE_PLAYER_DBL_CLICKED, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_LIST) ctrlAddEventHandler ["TreeDblClick", {
				["TreeDblClicked", _this] call DISPLAY;
			}]];

			// The camera buttons event handlers
			{
				(_display displayCtrl _x) ctrlAddEventHandler ["ButtonClick", {
					["CamButtonClicked", _this] call DISPLAY;
				}];
			...
			if (!(missionNamespace getVariable [VAR_ALLOW_FREE_CAMERA, true]) && {
				!(missionNamespace getVariable [VAR_ALLOW_3PP_CAMERA, true])
			}) then {
				...
				switch (["GetCameraMode"] call CAM) do {
					...
					case MODE_FOLLOW : {
						(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FOLLOW) ctrlSetText CAM_ICON_FOLLOW_SELECTED;
					};
					case MODE_FPS : {
						(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FPS) ctrlSetText CAM_ICON_FPS_SELECTED;
					};
				};
			};

			// Camera mode changes event handler
			uiNamespace setVariable [VAR_CAMERA_MODE_CHANGED, [missionNamespace, "EGSpectator_OnCameraModeChanged", {
				["SpectatorCameraModeChanged", _this] call DISPLAY;
			}] call BIS_fnc_addScriptedEventHandler];

			// Tabs event handlers
			uiNamespace setVariable [VAR_TABS_CLICKED, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_TABS) ctrlAddEventHandler ["ToolBoxSelChanged", {
				["OnTabSelected", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_TABS_MOUSE_ENTER, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_TABS) ctrlAddEventHandler ["MouseEnter", {
				["ShowList", _this] call DISPLAY;
			}]];
			uiNamespace setVariable [VAR_TABS_MOUSE_EXIT, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_TABS) ctrlAddEventHandler ["MouseExit", {
				["HideList", _this] call DISPLAY;
			}]];
			...
			// Header
			if !(missionNamespace getVariable [VAR_SHOW_HEADER, true]) then {
				(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_GAMETIMETEXT) ctrlShow false
			};

			// Camera buttons
			if !(missionNamespace getVariable [VAR_SHOW_CAMERA_BUTTONS, true]) then {
				(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_CAMERATYPESGROUP) ctrlShow false
			};
			...
			if !(missionNamespace getVariable [VAR_SHOW_LISTS, true]) then {
				(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_LIST) ctrlShow false;
				(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_TABS) ctrlShow false
			};
		...
		case "onUnload": {
			...
			if (!isNil {
				uiNamespace getVariable VAR_KEY_DOWN
			} && !isNil {
				uiNamespace getVariable VAR_DISPLAY
			} && {
				!isNull (uiNamespace getVariable VAR_DISPLAY)
			}) then {
				(uiNamespace getVariable [VAR_DISPLAY, displayNull]) displayRemoveEventHandler ["KeyDown", uiNamespace getVariable [VAR_KEY_DOWN, -1]];
			};

			if (!isNil {
				uiNamespace getVariable VAR_KEY_UP
			} && !isNil {
				uiNamespace getVariable VAR_DISPLAY
			} && {
				!isNull (uiNamespace getVariable VAR_DISPLAY)
		...
		case "Update_GameTime" : {
			...
			if (!isNull _display) then {
				...
				if (missionNamespace getVariable ["BIS_AK_vanguardSpectator", false]) then {
					...
					if (!isNil {
						missionNamespace getVariable "BIS_hvt_timeoutTarget"
					}) then {
		...
		case "Update_PlayerList" : {
			...
			// Go through groups and get the valid ones only, also cache group units information
			{
				...
				// Validate units
				{
					...
					} && !(_x isKindOf SPECTATOR_CLASS)) then {
						_unitsInfo pushBack [_x, alive _x, alive _x && {
							_x getVariable [VAR_INCAPACITATED, false]
						}, [_x, true, NAME_MAX_CHARACTERS] call BIS_fnc_getName, _group];
					};
				} forEach _units;
				...
				if !(_unitsInfo isEqualTo []) then {
					...
					switch (side _group) do {
						case west: {
							_west pushBack _result;
						};
						case east: {
							_east pushBack _result;
						};
						case resistance: {
							_indep pushBack _result;
						};
						default {
							_civ pushBack _result;
						};
					};
				};
			} forEach _groups;
			...
			if !(_oldList isEqualTo _newList) then {
				...
				// Update or add
				{
...

					{
						...
						private _groupIndex = if (count _i > 0) then {
							_i select 1
						} else {
							-1
						};
...

						{
							...
							private _text = if (isPlayer _unit) then {
								_name
							} else {
								format ["%1: %2", localize "str_player_ai", _name]
							};
							private _tooltip = if (isPlayer _unit) then {
								format ["%1 - %2", _name, _groupId]
							} else {
								format ["%1: %2 - %3", localize "str_player_ai", _name, _groupId]
							};
							private _i = ["TreeGetDataIndex", [[_unit] call BIS_fnc_objectVar]] call DISPLAY;
							private _unitIndex = if (count _i > 0) then {
								_i select 1
							} else {
								-1
							};
							...
							private _texture = switch (true) do {
								...
								case (_isIncapacitated) : {
									ICON_REVIVE
								};
								default {
									if (_unitIcon != "") then {
										getText (configfile >> "CfgVehicleIcons" >> _unitIcon)
									} else {
										""
									}
								};
		...
		case "Update_LocationList" : {
			...
			if !(_oldList isEqualTo _newList) then {
				...
				{
					["TreeDeleteItem", [_x]] call DISPLAY;
		...
		case "ShowFocusInfoWidget" : {
			...
			private _display = uiNamespace getVariable [VAR_DISPLAY, displayNull];
			private _widget = if (!isNull _display) then {
		...
		case "Update_FocusInfoWidget" : {
			...
			if (!isNull _focus && {
				missionNamespace getVariable [VAR_SHOW_FOCUS_INFO, true]
			} && {
				uiNamespace getVariable [VAR_FOCUS_WIDGET_VISIBLE, false]
			}) then {
				...
				_name = if (isPlayer _focus) then {
					[_focus, nil, NAME_MAX_CHARACTERS] call BIS_fnc_getName;
				} else {
					format ["%1: %2", localize "str_player_ai", [_focus, nil, NAME_MAX_CHARACTERS] call BIS_fnc_getName];
				};
				_unitIcon = getText (configfile >> "CfgVehicles" >> typeOf _focus >> "icon");
				_unitTypePicture = if (_unitIcon != "") then {
					getText (configfile >> "CfgVehicleIcons" >> _unitIcon)
				} else {
					""
				};
				...
				if (vehicle _focus == _focus) then {
					_ctrlUnitType ctrlSetText _unitTypePicture;
				} else {
					_ctrlUnitType ctrlSetText "";
				};
				_ctrlUnitType ctrlShow (vehicle _focus == _focus);
				if (vehicle _focus != _focus) then {
					_ctrlVehicleType ctrlSetText _vehicleTypePicture;
				} else {
					_ctrlVehicleType ctrlSetText "";
				};
		...
		case "UpdateCursorObject" : {
			...
			if ((_whitelist isEqualTo [] || {
				side group _cursorObject in _whitelist
			}) && {
				_allowAi || {
					isPlayer _cursorObject
				}
			} && {
				isNil {
					_cursorObject getVariable VAR_EXCLUDED_UNIT
				}
			}) then {
			...
			if (!isNull _cursorObject && {
				_oldCursorObject != _cursorObject
			} && {
				_whitelist isEqualTo [] || {
					side group _cursorObject in _whitelist
				}
			}) then {
		...
		case "SetFocus" : {
			...
			if (_newFocus != _oldFocus && {
				...
				if(!isNull _newFocus && {
					missionNamespace getVariable ["BIS_AK_vanguardSpectator", false]
				}) then {
		...
		case "ToggleUiVisibility" : {
			...
			private "_bVisible";
			_bVisible = if (uiNamespace getVariable [VAR_INTERFACE_VISIBLE, true]) then {
				false
			} else {
				true
			};
			...
			{
				private "_fade";
				_fade = if (_bVisible) then {
					getNumber (configFile >> "RscDisplayEGSpectator" >> "Controls" >> ctrlClassName _x >> "fade");
				} else {
					1.0
				};
		...
		case "KeyDown": {
			...
			switch true do {
				...
				case (_key == DIK_BACKSPACE): {
					["ToggleUiVisibility"] call DISPLAY;
					true;
				};
				case (_key == DIK_F1): {
					["ToggleControlsHelp"] call DISPLAY;
					true;
				};
				case (_key == DIK_PGUP): {
					["AddViewDistanceInput", [250.0]] call DISPLAY;
					true;
				};
				case (_key == DIK_PGDN): {
					["AddViewDistanceInput", [-250.0]] call DISPLAY;
				...
				case (_key == DIK_RIGHT): {
					["OnSwitchFocusClicked", [true]] call DISPLAY;
					true;
				};
				case (_key == DIK_LEFT): {
					["OnSwitchFocusClicked", [false]] call DISPLAY;
					true;
				};
				case (_key == DIK_N): {
					["ToggleVisionMode"] call CAM;
					true;
				};
				case (_key == DIK_SPACE): {
					["ToggleCameraMode"] call CAM;
					true;
				};
				...
				case (_key == DIK_L): {
					["ToggleCameraLights"] call CAM;
					true;
				};
				case (_key == DIK_LALT): {
					["ToggleVeryFastSpeed", [true]] call CAM;
					true;
				};
				case (_key == DIK_O): {
					missionNamespace setVariable [VAR_DRAW_PROJECTILES_PATH, !(missionNamespace getVariable [VAR_DRAW_PROJECTILES_PATH, false])];
					true;
				};
				default {
					false;
		...
		case "KeyUp": {
			...
			switch true do {
				case (_key == DIK_LALT): {
					["ToggleVeryFastSpeed", [false]] call CAM;
					true;
				};
				default {
					false;
		...
		case "Interrupt" : {
			if (uiNamespace getVariable [VAR_MAP_VISIBLE, false]) then {
				...
				private _display = uiNamespace getVariable [VAR_DISPLAY, displayNull];
				private _displayType = if (isMultiplayer) then {
		...
		case "ToggleMap" : {
			...
			if (!isNull _display) then {
				if !(["IsMapVisible"] call DISPLAY) then {
					...
					uiNamespace setVariable [VAR_MAP_CLICKED, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MAP) ctrlAddEventHandler ["MouseButtonClick", {
						["OnMapClicked", _this] call DISPLAY;
					}]];
					uiNamespace setVariable [VAR_MAP_DRAW, (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MAP) ctrlAddEventHandler ["Draw", {
						_this call SPEC_DRAW2D;
						["DrawMap", _this] call DISPLAY;
					}]];
				} else {
					uiNamespace setVariable [VAR_MAP_VISIBLE, false];
					(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_MAPGROUP) ctrlShow false;
		...
		case "MouseButtonDown" : {
			...
			switch (_key) do {
				...
				case 1 : {
					...
					if (_cameraMode == MODE_FREE && {
						!isNull _focus
					} && {
						!isNull (attachedTo _dummy)
					}) then {
						[] call CAM_RESET_TARGET;
					};
				};
		...
		case "MouseMoving" : {
			if (["GetCameraMode"] call CAM == MODE_FOLLOW && {
				uiNamespace getVariable [VAR_IS_HOLDING_RIGHT_M_BUTTON, false]
			}) then {
		...
		case "CamButtonClicked" : {
			...
			switch (_ctrl) do {
				case (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FREE) : {
					["SetCameraMode", [MODE_FREE]] call CAM;
				};
				case (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FOLLOW) : {
					["SetCameraMode", [MODE_FOLLOW]] call CAM;
				};
				case (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FPS) : {
					["SetCameraMode", [MODE_FPS]] call CAM;
				};
			};
		};
		...
		case "ResetCameraButtonsState" : {
...

			{
				...
				switch (_control) do {
					case (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FREE) : {
						_control ctrlSetText CAM_ICON_FREE;
					};
					case (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FOLLOW) : {
						_control ctrlSetText CAM_ICON_FOLLOW;
					};
					case (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FPS) : {
						_control ctrlSetText CAM_ICON_FPS;
					};
				};
			} forEach [IDC_RSCDISPLAYEGSPECTATOR_FREE, IDC_RSCDISPLAYEGSPECTATOR_FOLLOW, IDC_RSCDISPLAYEGSPECTATOR_FPS];
		};
		...
		case "SetSideTask" : {
			...
			_display = uiNamespace getVariable [VAR_DISPLAY, displayNull];
			_control = _display displayCtrl (if (_side == 0) then {
				IDC_RSCDISPLAYEGSPECTATOR_SIDEATASK
			} else {
				IDC_RSCDISPLAYEGSPECTATOR_SIDEBTASK
			});
		...
		case "FlashSideTaskThread" : {
			...
			private _idcBackground = if (_side == 0) then {
				IDC_RSCDISPLAYEGSPECTATOR_SIDEABACKGROUND
			} else {
				IDC_RSCDISPLAYEGSPECTATOR_SIDEBBACKGROUND
			};
			private _idcText = if (_side == 0) then {
				IDC_RSCDISPLAYEGSPECTATOR_SIDEATASK
			} else {
				IDC_RSCDISPLAYEGSPECTATOR_SIDEBTASK
			};
		...
		case "SetControlsHelpVisibility" : {
			...
			if (_bVisible && {
				missionNamespace getVariable [VAR_SHOW_CONTROLS_HELPER, true]
			}) then {
		...
		case "UpdateControlsHelp" : {
			...
			if (_cameraMode != MODE_FPS && {
				_cameraMode != MODE_FREE || {
					_allowFreeCamera
				}
			}) then {
