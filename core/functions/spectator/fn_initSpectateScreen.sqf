...
/* private _playerSide = GETPLVAR(BriefingSide, west); */
	...
	[{
		!(_this select 0 isEqualTo controlNull) && {
			!(_this select 1 isEqualTo controlNull)
		}
	}, {
		params ["_teamCombo", "_entryList"];
		
		 _teamCombo ctrlAddEventHandler ["LBSelChanged", {
			["COMBOCHANGE", _this] call FUNC(spectatorBriefing)
		}];
		
		 _entryList ctrlAddEventHandler ["LBSelChanged", {
			["LISTCHANGE", _this] call FUNC(spectatorBriefing)
		}];
	...
	private _fnc_addControls = {
		params ["_help", "_background"];
		 ...
		 private _hasFocus = !isNull (["GetFocus"] call {
			_this call (uiNamespace getVariable ["RscDisplayEGSpectator_script", {}])
		});
		
			private _cameraMode = ["GetCameraMode"] call {
	...
	private _observeHelpControlsPFH = [{
		params ["_args", "_idPFH"];
		 ...
		 if (_display isEqualTo displayNull || {
			_help isEqualTo controlNull
		}) exitWith {
