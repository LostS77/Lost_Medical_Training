...
if !(_cam isEqualTo objNull) then {
	[{
		...
		GVAR(eg_keyHandle) = (findDisplay 60492) displayAddEventHandler ["keyDown", {
			call FUNC(eg_keyHandler);
		}];
		GVAR(eg_keyHandle2) = (findDisplay 46) displayAddEventHandler ["keyDown", {
			call FUNC(eg_keyHandler2)
		}];
	...
	if (GETMVAR(killcam_active, true)) then {
		...
		[{
			...
			GVAR(killcam_keyHandle) = (findDisplay 60492) displayAddEventHandler ["keyDown", {
				call FUNC(killcam_toggleFnc);
			}];
		}, []] call CBA_fnc_waitUntilAndExecute;
