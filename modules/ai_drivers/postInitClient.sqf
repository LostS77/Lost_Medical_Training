...
if (GETMVAR(TestMode, false)) then {
	...
	private _pipAction = ["ai_driver_pip", "Enable/Disable driver's view", "", {
		(isNil QGVAR(DriverCam) || {
			isNull GVAR(DriverCam)
		}) call FUNC(ToggleDriverCam);
	}, {
		(vehicle _player == _target) &&
		{
			effectiveCommander vehicle _player isEqualTo _player
	...
	private _pipNvAction = ["ai_driver_pip_nv", "Enable/Disable NV in driver's view", "", {
		...
		{
			{
				_x in (items (GETVAR(_target, driver, objNull)) + assignedItems (GETVAR(_target, driver, objNull)))
			} count ["NVGoggles", "NVGoggles_OPFOR", "NVGoggles_INDEP"] > 0
		} &&
		{
			!isNil QGVAR(DriverCam) && {
				!isNull GVAR(DriverCam)
	...
	if (GETMVAR(AllCars, false)) then {
		...
		_vehClasses = _vehClasses select {
			!(_x isKindOf "Car")
		};
		_vehClasses pushBackUnique "Car";
		_selectVehicles = _selectVehicles select {
			!(_x isKindOf "Car")
		};
	};
