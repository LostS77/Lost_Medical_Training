...
[{
	...
	private _actionVehEH = [vehicle ACE_player, "DefaultAction", {
		true
	}, {
	...
	GVAR(distance) = switch (side ACE_player) do {
		case blufor: {
			GVAR(BLUFOR_Distance)
		};
		case opfor: {
			GVAR(OPFOR_Distance)
		};
		case independent: {
			GVAR(INDFOR_Distance)
		};
		case civilian: {
			GVAR(CIV_Distance)
		};
	...
	[{
		((GETMVAR(time, 30)) <= 0 || {
			CBA_missionTime >= GETMVAR(time, 30)
		})
		|| {
			(GETMVAR(distance, 200)) <= 0 || {
				((EGETMVAR(FW, SpawnPos, getPos (vehicle ACE_player))) distance (vehicle ACE_player)) >= GETMVAR(distance, 200)
			}
		}
	}, {
